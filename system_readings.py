...
def write_RAM_usage(data_dict):
    data_dict["RAM"] = []
    data_dict["RAM"].append("{:.2f}".format(psutil.virtual_memory()[0]/(10**9))) #total
    data_dict["RAM"].append("{:.2f}".format(psutil.virtual_memory()[3]/(10**9))) #used
    data_dict["RAM"].append("{:.2f}".format(psutil.virtual_memory()[1]/(10**9))) #available
